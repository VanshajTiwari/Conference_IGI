...
exports.sendMessage = async (req, res) => {
	...
	try {
		const findConversation = await conversation.findOne({
			chatters: { $all: [receiver, req.session.user.id] },
		});
		if (!findConversation) {
...
exports.superChattingControllers = async (req, res) => {
	...
	try {
		...
		let newChat = await conversation.findOne({
			chatters: { $all: ['super', 'chats'] },
		});
...
exports.getSuperChats = async (req, res) => {
	let superChat = await conversation
		.findOne({ chatters: { $all: ['super', 'chats'] } })
		.populate({ path: 'messageDetails', populate: { path: 'senderId' } });
	if (!superChat) {
		return;
	}
	res.status(200).json({
		status: 'success',
		chats: superChat.messageDetails,
	});
};
